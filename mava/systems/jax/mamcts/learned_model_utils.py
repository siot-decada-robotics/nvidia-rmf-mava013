...
from rlax._src.transforms import signed_hyperbolic, signed_parabolic, transform_from_2hot, transform_to_2hot
...
def normalise_encoded_state(encoded_state, epsilon=1e-5):
    ...
    scale_encoded_state = jnp.where(scale_encoded_state<epsilon,scale_encoded_state+epsilon,scale_encoded_state)
...
def join_flattened_observation_action_history(stacked_observation_history,stacked_action_history,num_actions):
    ...
    full_history = jnp.concatenate((stacked_observation_history.reshape(1,-1),stacked_action_history.reshape(1,-1)),axis=-1)
    
    return full_history

def join_non_flattened_observation_action_history(stacked_observation_history, stacked_action_history, num_actions):
    """Process and concatenate observation and action history for non-single dimension observations"""
    stacked_observation_history = jnp.transpose(stacked_observation_history,axes=(*range(1,len(stacked_observation_history.shape)),0)) # expects no batch dimension
    stacked_action_history = actions_to_tiles(stacked_action_history,stacked_observation_history.shape[:-1],num_actions)
    stacked_action_history = jnp.squeeze(stacked_action_history,axis=-1)
    stacked_action_history = jnp.transpose(stacked_action_history,axes=(*range(1,len(stacked_action_history.shape)),0))

    full_history = jnp.concatenate((stacked_observation_history,stacked_action_history),axis=-1)

    return full_history

def pad_history(stacked_observation_history, stacked_action_history, history_size):
    """Add zero padding to observation and action history"""

    padded_hist_size = history_size - stacked_observation_history.shape[0]
    
    padded_obs = jnp.zeros((1,*stacked_observation_history.shape[1:]),stacked_observation_history.dtype)
   
    padded_obs = jnp.repeat(padded_obs, padded_hist_size, axis=0)
   
    stacked_observation_history = jnp.concatenate([padded_obs, stacked_observation_history], axis=0)

    padded_actions = jnp.zeros((1,*stacked_action_history.shape[1:]),stacked_action_history.dtype,)
