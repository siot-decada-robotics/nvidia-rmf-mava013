...
def create_and_run_lp_program(config: Dict) -> None:
    ...
    system.build(
        ...
        multi_process=True,
        clip_value=False,
        termination_condition={"executor_steps": 100000},
    )
    # Launch the system.
...
def mp_process_lp_program(config: Dict) -> None:
    """Code to wrap lp program in a process.

    Args:
        config : hyperparam config.
    """
    p = mp.Process(target=create_and_run_lp_program, args=(config,))
    p.start()
    p.join()  # blocks till process is done.
...
def main(_: Any) -> None:
    ...
    Args:
        _ : unused.
    """
    # Try to terminate program for 30s
    FLAGS.lp_termination_notice_secs = 30
    config = {}
    # Loop through a hyperparam such as num_epochs
    for num_epochs in [5, 10, 15]:
        config["num_epochs"] = num_epochs
        mp_process_lp_program(config)
        # NB Termination using the current version of lp (0.5.0) results in an error.
        # This was fixed in dm-launchpad-nightly==0.5.2.dev20220817. Once a new version
        # of lp has been released, and we add `FLAGS.lp_worker_manager_v2 = True`,
